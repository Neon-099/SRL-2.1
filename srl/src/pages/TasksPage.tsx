...
import { TaskFormModal } from '../components/modal/TaskFormModal.tsx';
import { TaskEditModal } from '../components/modal/TaskEditModal.tsx';
export default function TasksPage() {
    const [isCreateModalOpen, setIsCreateModalOpen] = useState(false);
    const [isEditModalOpen, setIsEditModalOpen] = useState(false);
    const [editingTask, setEditingIncident] = useState<Task | null>(null);
    const [searchQuery, setSearchQuery] = useState('');

    const { tasks, loading, deleted } = useTasks();
    ...
    const handleEditIncident = (task: Task) => {
        setEditingIncident(task);
    ...
    console.log("Filtered Incidents: ", tasks);
                    ...
                    <div className="flex justify-between items-start mb-4">
                        <div>
                            ...
                            <p className="text-sm text-gray-500">
                                {tasks.length} {tasks.length === 1 ? 'item' : 'items'} found
                ...
                {loading ? (
                    ...
                    <div className="text-center py-16 max-w-md mx-auto">
                        ...
                        <p className="text-xl font-semibold text-gray-700 mb-2">
                            No {tasks.length === 0 ? 'tasks' : tasks + 's'} found
                        </p>
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex justify-between items-start mb-3">
                                            <div className="flex-1 min-w-0 pr-4">
                                                ...
                                                <div className="flex items-center gap-2 flex-wrap">
                                                    ...
                                                    {getIncidentSyncBadge(task.synced)}
                                                </div>
                                            </div>
                                            <div className="flex gap-1.5">
                                                ...
                                                <button
                                                    onClick={() => handleDeleteIncident(task.id)}
                                        ...
                                        {task.photos && task.photos.length > 0 && (
                                            <div className="flex items-center gap-2 text-sm text-gray-600 bg-gray-50 px-3 py-2 rounded-lg inline-flex">
                                                ...
                                                <span>photo{task.photos.length > 1 ? 's' : ''} attached</span>
                                            </div>
                                        )}
                                    </div>
                                </div>
